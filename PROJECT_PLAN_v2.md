...
- v2 → v2.1 (구현 진행 중 결정, 2026-04): 품목에서 매입단가(`purchase_price`) 컬럼 제외, 매입에서 거래처(`supplier_name`) 컬럼 영구 제외. 결정 근거와 세부 사항은 `CLAUDE.md` 의 작업 로그 참조. 본 문서의 스키마 정의는 추후 실제 구현에 맞춰 동기화 예정.
- v2.1 → v2.2 (2026-05): 매입에서 매입금액(`amount`) 컬럼 영구 제외 — 수량과 품목만 추적. 매입 등록 화면에 품목명 자동완성(`<datalist>`) UX 도입.
